...
public class LinkedDocument extends Document {
    ...
    private void createOutgoingDocumentCollection() {
        ...
        for (String outgoingID : this.outgoingIDs) {
            LinkedDocument newDoc = LinkedDocument.createLinkedDocumentFromFile(outgoingID);
    ...
    private void createOutgoingDocumentCollection(LinkedDocumentCollection cache) {
        ...
        for (String outgoingID : this.outgoingIDs) {
            newDoc = cache.findByID(outgoingID);

            if (newDoc == null)
                newDoc = LinkedDocument.createLinkedDocumentFromFile(outgoingID);
    ...
    private String[] findOutgoingIDs(String text) {
        ...
        if (noOfIDs >= 0) System.arraycopy(tmpIDs, 0, ids, 0, noOfIDs);
