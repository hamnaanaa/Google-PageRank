...
public class WordCountsArray {
    ...
    private void doBucketSort() {
        ...
        for (int pos = 1; pos < maxLength; pos++) {
            ...
            for (WordCountsArray bucket : buckets) {
                for (int j = 0; j < bucket.size(); j++) {
                    String word = bucket.getWord(j);
                    int charIndex = bucketAt(word, maxLength, pos);
                    int count = bucket.getCount(j);
        ...
        int j = 0;
        for (WordCountsArray wordCountsArray : buckets) {
            for (int i = 0; i < wordCountsArray.size(); i++) {
                newWordCounts[j] = wordCountsArray.get(i);
                j++;
            }
        }
    ...
    public double computeSimilarity(WordCountsArray wca, DocumentCollection dc) {
        ...
        // Force wca to compute normalized weights otherwise scalar product is always 0
    ...
    private void doubleSize() {
        ...
        /* copy old array */
        System.arraycopy(this.wordCounts, 0, newWordCounts, 0, this.wordCounts.length);
    ...
    private double scalarProduct(WordCountsArray wca, DocumentCollection dc) {
        if (wca == null || dc == null) {
            System.out.println("(wca == null || dc == null)");
            return 0;
        }

        /* scalar product is 0 by definition, if size is different */
        if (this.size() != wca.size()) {
            System.out.println("(this.size() != wca.size()): " + size() + " != " + wca.size());
            return 0;
        }
         ...
         */
        if ((this != wca) && !this.wordsEqual(wca)) {
            System.out.println("((this != wca) && !this.wordsEqual(wca))");
            return 0;
        }
